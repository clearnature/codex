    ...
    for spec in tools {
        match spec {
            ...
            DynamicToolSpec::Namespace(namespace) => {
                ...
                if name.is_empty() {
                    return Err(
                        tr(current(), "dynamic tool namespace must not be empty").to_string()
                    );
                }
    ...
    pub(crate) async fn thread_section_move(
...

        {
            ...
            self.thread_manager
                ...
                .await
                .map_err(|err| {
                    core_thread_write_error(tr(current(), "move thread in section"), err)
                })?;
        }
    ...
    async fn thread_metadata_update_response_inner(
            ...
            .map(
                |ThreadMetadataGitInfoUpdateParams {
                    ...
                    let origin_url = match origin_url {
                        Some(Some(origin_url)) => Some(Some(
                            SanitizedGitUrl::try_from(origin_url).map_err(|_| {
                                invalid_request(tr(
                                    current(),
        ...
        let updated_thread = {
            ...
            let updated_thread = self
                ...
                .await
                .map_err(|err| {
                    core_thread_write_error(tr(current(), "update thread metadata"), err)
                })?;
    ...
    async fn reload_paginated_thread(
        ...
        if resumed_thread_id != thread_id {
            return Err(internal_error(tr_with(
                ...
                &[&thread_id.to_string(), &resumed_thread_id.to_string()],
            )));
        }
        codex_thread
    ...
    async fn thread_fork_inner(
            ...
            let history_items = match (last_turn_id.as_deref(), before_turn_id.as_deref()) {
                (Some(last_turn_id), None) => {
                    truncate_rollout_after_turn_id(source_history_items, last_turn_id).map_err(
                        |err| {
                            core_thread_write_error(tr(current(), "truncate thread for fork"), err)
                        },
                    )?
                }
                (None, Some(before_turn_id)) => {
                    truncate_rollout_before_turn_id(source_history_items, before_turn_id).map_err(
                        |err| {
                            core_thread_write_error(tr(current(), "truncate thread for fork"), err)
                        },
                    )?
            ...
            && let Some(name) = source_thread_name.clone()
        {
            self.thread_manager
                ...
                .await
                .map_err(|err| {
                    core_thread_write_error(tr(current(), "inherit source thread name"), err)
                })?;
        }
...
pub(super) fn core_thread_write_error(operation: &str, err: CodexErr) -> JSONRPCErrorError {
    match err.details() {
        CodexErrorDetails::ThreadNotFound(thread_id) => invalid_request(tr_with(
            current(),
            "thread not found: {0}",
            &[&thread_id.to_string()],
        )),
