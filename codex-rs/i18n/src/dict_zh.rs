...
pub(crate) static ENTRIES: &[(&str, &str)] = &[
    ...
    ("[ . ] Action Required", "[ . ] 需要操作"),
    ("a user-configured MCP server already owns the codex_tui namespace", "用户配置的MCP服务器已占用codex_tui命名空间"),
    ...
    ("failed to encode thread/inject_items payload", "无法编码thread/inject_items载荷"),
    ("failed to list loaded threads from app server", "无法从应用服务器列出已加载的线程"),
    ("failed to serialize Auto Review denial event", "无法序列化Auto Review拒绝事件"),
    ("failed to unarchive session", "无法取消归档会话"),
    ("forked_from_id is invalid: {0}", "forked_from_id无效：{0}"),
    ("item", "条目"),
    ("items", "条目"),
    ("managed MCP requirements do not permit the TUI task-tools server", "托管MCP要求不允许TUI task-tools服务器"),
    ("not started", "未启动"),
    ("raw output", "原始输出"),
    ("starting", "正在启动"),
