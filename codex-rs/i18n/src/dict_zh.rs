...
pub(crate) static ENTRIES: &[(&str, &str)] = &[
    ...
    ("A steady rock when the diff gets large", "diff变大时的稳如磐石"),
    ("A tidy duck for calm workspace days", "平静工作日里的整洁小鸭"),
    ...
    ("Not found", "未找到"),
    ("Pending init", "等待初始化"),
    ("Pets are disabled in Zellij. Terminal images don’t stay reliably pane-local in Zellij. Run Codex outside Zellij to use pets.", "Zellij中已禁用宠物。终端图像在Zellij中无法可靠地局限于单个窗格。请在Zellij之外运行Codex以使用宠物。"),
    ("Pets are disabled in tmux. Terminal images don’t stay pane-local in tmux and can corrupt scrollback or move between panes. Run Codex outside tmux to use pets.", "tmux中已禁用宠物。终端图像在tmux中无法局限于单个窗格，可能破坏回滚缓冲或在窗格间移动。请在tmux之外运行Codex以使用宠物。"),
    ("Pets aren’t available in this terminal. Terminal pets need image support, and this terminal environment doesn’t expose a supported image protocol. Try a terminal with Kitty graphics or Sixel support, or run Codex outside tmux.", "此终端不支持宠物。终端宠物需要图像支持，而当前终端环境未提供受支持的图像协议。请改用支持Kitty图形或Sixel的终端，或在tmux之外运行Codex。"),
    ("Pets require iTerm2 3.6 or newer. Upgrade iTerm2 to use terminal pets.", "宠物需要iTerm2 3.6或更高版本。请升级iTerm2以使用终端宠物。"),
    ...
    ("animation {0} fallback {1} does not exist", "动画{0}的回退{1}不存在"),
    ("animation {0} fps must be finite and between 0 and {1}, got {2}", "动画{0}的fps必须是有限值且介于0与{1}之间，实际为{2}"),
    ("animation {0} references sprite index {1}, but pet has {2} frames", "动画{0}引用了精灵索引{1}，但宠物只有{2}帧"),
    ("authentication required", "需要认证"),
    ("base64 payload is not valid UTF-8", "base64载荷不是有效的UTF-8"),
    ("cancelled", "已取消"),
    ...
    ("frame path has no valid file stem", "帧路径没有有效的文件名主干"),
    ("install {0}", "安装{0}"),
    ("item", "条目"),
    ("items", "条目"),
    (
    ...
    ("join pet spritesheet cache validation task", "等待宠物精灵表缓存校验任务"),
    ("join pet spritesheet install task", "等待宠物精灵表安装任务"),
    ("not started", "未启动"),
    ("parse pet asset download URL {0}", "解析宠物资源下载URL{0}"),
    ("pet asset download from {0} exceeded {1} bytes", "从{0}下载的宠物资源超过{1}字节"),
    ("pet frame index does not fit usize", "宠物帧索引无法放入usize"),
    ("pet frame index exceeds expected frame count", "宠物帧索引超出预期的帧数"),
    ("pet frame index overflow", "宠物帧索引溢出"),
    ("pet frame x offset overflow", "宠物帧x偏移溢出"),
    ("pet frame y offset overflow", "宠物帧y偏移溢出"),
    ("pet image asset unavailable: {0}", "宠物图像资源不可用：{0}"),
    ("pet spritesheet path should include an assets directory", "宠物精灵表路径应包含assets目录"),
    ("raw output", "原始输出"),
    ("read pet asset download from {0}", "读取从{0}下载的宠物资源"),
    ("read {0}", "读取{0}"),
    ("remove {0}", "删除{0}"),
    ("sixel RGBA buffer length overflow", "sixel RGBA缓冲区长度溢出"),
    ("sixel byte index does not fit usize", "sixel字节索引无法放入usize"),
    ("sixel byte index overflow", "sixel字节索引溢出"),
    ("sixel image dimensions must be non-zero", "sixel图像尺寸必须非零"),
    ("sixel pixel count does not fit usize", "sixel像素计数无法放入usize"),
    ...
    ("tools", "工具"),
    ("unknown", "未知"),
    ("unknown protocol {0}; expected auto, kitty, or sixel", "未知协议{0}；应为auto、kitty或sixel"),
    ("unsupported pet asset download URL scheme {0}", "不支持的宠物资源下载URL协议{0}"),
    ("write {0}", "写入{0}"),
    ("{0} actions.", "{0}个操作。"),
    ("{0} credits", "{0} 额度"),
    ("{0} window", "{0} 窗口"),
