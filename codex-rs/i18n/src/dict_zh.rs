...
pub(crate) static ENTRIES: &[(&str, &str)] = &[
    ...
    ("Active background terminals block /cd.", "有活动的后台终端，/cd被阻止。"),
    ("Agent errored", "代理出错"),
    ("Agent resume failed", "代理恢复失败"),
    ("Agent spawn failed", "代理启动失败"),
    ("Ask a workspace admin to confirm plugin access.", "请联系工作区管理员确认插件访问权限。"),
    ("Ask a workspace admin to enable Codex plugins or plugin sharing.", "请联系工作区管理员启用Codex插件或插件共享。"),
    ("Ask the sharer or a workspace admin to confirm plugin access.", "请联系共享者或工作区管理员确认插件访问权限。"),
    ("Blocked", "已阻塞"),
    (
        "Choose a pet to wake in the terminal.",
        "选择要在终端中唤醒的宠物。",
    ),
    ("Cannot continue into the new worktree because source configuration changed.", "源配置已变更，无法继续进入新的工作树。"),
    ("Cannot continue into the new worktree while the source session changed or is unavailable.", "源会话已变更或不可用，无法继续进入新的工作树。"),
    ("Cannot create a worktree from an explicitly untrusted source.", "无法从显式不受信任的来源创建工作树。"),
    ("Cannot load the new worktree configuration: {0}", "无法加载新的工作树配置：{0}"),
    ("Check that you are signed in to the correct workspace and still have access.", "请确认你已登录正确的工作区且仍拥有访问权限。"),
    ...
    ("Creating a worktree requires an idle primary session without queued input.", "创建工作树需要主会话空闲且没有排队输入。"),
    ...
    ("MCP Tools", "MCP工具"),
    ("MDM value", "MDM值"),
    ("Managed worktrees are only supported for local sessions.", "托管工作树仅支持本地会话。"),
    ("Needs input", "需要输入"),
    ...
    ("Plugin sharing is disabled for this Codex session. Enable plugin sharing to load shared plugins.", "本次Codex会话已禁用插件共享。请启用插件共享以加载共享插件。"),
    ...
    ("Shutdown", "已停止"),
    ("Sign in to ChatGPT, then try loading this section again.", "请先登录ChatGPT，然后重新加载此分区。"),
    ("Sign in with ChatGPT auth; API key auth cannot load remote plugin catalogs.", "请用ChatGPT账号登录；API key认证无法加载远程插件目录。"),
    ...
    ("Switch to the matching workspace or ask the sharer for access.", "请切换到匹配的工作区，或向共享者申请访问权限。"),
    ("Tasks {0}/{1}", "任务 {0}/{1}"),
    ("Terminal pets disabled", "终端宠物已禁用"),
    ("The new worktree is not trusted; run Codex there.", "新的工作树尚未受信任；请在那里运行Codex。"),
    ("The original Codex companion", "最初的Codex伙伴"),
    ("Thinking", "思考中"),
    ("Tool {0}", "工具{0}"),
    ("Try again later; local plugin functionality is still available.", "请稍后重试；本地插件功能仍可使用。"),
    ("Type to filter pets...", "输入以筛选宠物…"),
    ("Type to search shortcuts", "输入以搜索快捷键"),
    ("Unbound ({0})", "未绑定（{0}）"),
    ("Update Codex, then try opening the shared plugin again.", "请更新Codex，然后重试打开该共享插件。"),
    ...
    ("plugin list cwd must be absolute", "插件列表的cwd必须是绝对路径"),
    ("raw output", "原始输出"),
    (
    ...
    ("thread usage request timed out in TUI", "线程用量请求在TUI中超时"),
    ("tool", "工具"),
    ("tools", "工具"),
    ("unknown", "未知"),
    (
    ...
    ("write {0}", "写入{0}"),
    ("{0} A checkout was retained at {1}; remove it with `git worktree remove <checkout-path>` from the source repository if it is no longer needed.", "{0}已在{1}保留一份检出；如不再需要，请在源仓库中运行 `git worktree remove <checkout-path>` 将其删除。"),
    ("{0} actions.", "{0}个操作。"),
    ("{0} credits", "{0} 额度"),
    ("{0} window", "{0} 窗口"),
