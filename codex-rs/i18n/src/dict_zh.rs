...
pub(crate) static ENTRIES: &[(&str, &str)] = &[
    ...
    ("  <none>", "  <无>"),
    ("  No experimental features available for now", "  目前没有可用的实验性特性"),
    ...
    ("Checked features are configured on. Some experimental features take effect only in new tasks or after restarting the Codex server.", "已勾选的特性将配置为开启。部分实验性特性仅在新建任务或重启Codex服务器后生效。"),
    ("Choose how Codex uses and creates memories. Changes are saved to config.toml", "选择Codex如何使用与创建记忆。更改会保存到config.toml"),
    ...
    ("Clear local memory files and summaries. Existing threads stay intact.", "清除本地记忆文件与摘要。现有线程不受影响。"),
    ("Closed", "已关闭"),
    ...
    ("Delete local memory files and rollout summaries.", "删除本地记忆文件与rollout摘要。"),
    ...
    ("Generate memories from the following threads. Current thread included.", "从以下线程生成记忆。包含当前线程。"),
    ...
    ("Saving was interrupted. Reopen /experimental to check configured values.", "保存被中断。请重新打开/experimental检查已配置的值。"),
    ("Saving… Closing this popup will not cancel the write.", "正在保存…关闭此弹窗不会取消写入。"),
    ("Search › ", "搜索 › "),
    ("Select Pet", "选择宠物"),
    ("Selected {0} of {1} {2}.", "已选择{0}/{1}个{2}。"),
    ("Selections retained. Save to retry, or cancel to close.", "已保留选择。保存以重试，或取消以关闭。"),
    ...
    ("This clears local memory files and rollout summaries for the current Codex home.", "这将清除当前Codex主目录的本地记忆文件与rollout摘要。"),
    ...
    ("Use memories", "使用记忆"),
    ("Use memories in the following threads. Applied at next thread.", "在以下线程中使用记忆。在下一个线程生效。"),
    ("Viewed {0}", "已查看{0}"),
