...
**平台受限站点合计 45 个**（其中 **43 处是代码改动**：mac.rs 33 + windows.rs 6 + debug_sandbox 的 2 处 macOS 分支与 2 处 Windows 分支；另 2 个是登记行）。本表列出三个相关文件的全部 52 个站点以便对照 —— **注意**：`debug_sandbox.rs` 的另外 10 处在 Linux 编译路径上，已由 clippy/测试覆盖，不在「未验证」之列。
