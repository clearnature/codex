...
**同批另两次「门禁抓到人」（都留了失败回执）**：① `clippy` 报 `E0618` —— 我在 `events.rs` 的替换里
把 `.to_string()` 写成了 `.to_string()()`（替换文本与原文尾部重叠），失败回执 `r-mu7w6n3b-xhmors`；
② 修好后又报 `clippy::unnecessary_to_owned`（`&feature.key().to_string()` 多余，`feature.key()` 已是 `&str`），
